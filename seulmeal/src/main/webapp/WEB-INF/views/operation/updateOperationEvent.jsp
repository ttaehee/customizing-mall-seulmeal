...
<div class="container">
	...
	<div class="container">
	<form id="insertForm" action="/operation/updateOperation" method="POST" enctype="multipart/form-data">
		...
		<div class="row">
			...
			<input type="hidden" value="${post.thumnail}" name="thumnail" />
			<div class="col-md-12" style="margin-top:20px;" >
				<div class="filebox" style="display: flex; justify-content:space-around;">
						<input class="upload-name" value="${post.thumnail }" id="upload-name" name="checkThumnail" readonly style="width:90%;">
			...
			<div id="productClass" class="col-md-12" style="margin-top:20px;">
				...
				<c:forEach var="product" items="${post.discountProduct}">
					<c:if test="${product.productNo != 0 }">
						<div class="productCommponet">
							<span class='name'>${product.name}</span>
							<span class='oriPrice'>${product.price}</span>
							<span class='price'>${product.price}</span>
							<button type='button' class='btn btn-primary' data-value="${product.productNo}" onClick='deleteProduct(this)'>x</button>
						</div>
					</c:if>					
				</c:forEach>
			</div>
		...
		<div class="text-right" style="margin-top:20px;">
			<button type="button" onclick="updateEnvet()" class="btn btn-primary">
				등록
			</button>
...
<script>
	...
	function deleteProduct(e){
		deleteProductNo.push($(e).data("value"))
		$(e).parent().remove();
		console.log(deleteProductNo);
	}
	...
	function updateEnvet(){
		...
		const thumnail = $("#upload-name").val();
